...
init_status.icm <- function(dat) {
  ...
  p.num <- dat$init$p.num
  a.num <- dat$init$a.num
  ...
  p.num.g2 <- dat$init$p.num.g2
  a.num.g2 <- dat$init$a.num.g2
  ...
  if (type %in% c("SIR", "SEIR", "SEIQHR", "SEIQHRF", "SEIQHRFPA")) {
  ...
  if (type %in% c("SEIQHRFPA")) {
    status[sample(which(group == 1 & status == "s"), size = p.num)] <- "p"
    status[sample(which(group == 1 & status == "s"), size = a.num)] <- "a"
    if (nGroups == 2) {
      status[sample(which(group == 2 & status == "s"), size = p.num.g2)] <- "p"
      status[sample(which(group == 2 & status == "s"), size = a.num.g2)] <- "a"
    }
  }
