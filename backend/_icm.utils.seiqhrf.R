get_prev.seiqhrf.icm <- function(dat, at) {
  ...
  if (at == 1) {
    ...
    dat$epi$i.num <- sum(dat$attr$active == 1 &
                         dat$attr$status == "i" &
                         dat$attr$group == 1)
    if (dat$control$type %in% c("SEIR", "SEIQHR", "SEIQHRF", "SEIQHRFPA")) {
        dat$epi$e.num <- sum(dat$attr$active == 1 &
                             dat$attr$status == "e" &
                             dat$attr$group == 1)
    }        
    if (dat$control$type %in% c("SIR", "SEIR", "SEIQHR", "SEIQHRF", "SEIQHRFPA")) {
      dat$epi$r.num <- sum(dat$attr$active == 1 &
                           dat$attr$status == "r" &
                           dat$attr$group == 1)
    }
    if (dat$control$type %in% c("SEIQHR", "SEIQHRF", "SEIQHRFPA")) {
    ...
    if (dat$control$type %in% c("SEIQHRF", "SEIQHRFPA")) {
      dat$epi$f.num <- sum(dat$attr$active == 1 &
                           dat$attr$status == "f" &
                           dat$attr$group == 1)
    }
    if (dat$control$type %in% c("SEIQHRFPA")) {
      dat$epi$p.num <- sum(dat$attr$active == 1 &
                             dat$attr$status == "p" &
                             dat$attr$group == 1)
      dat$epi$a.num <- sum(dat$attr$active == 1 &
                             dat$attr$status == "a" &
                             dat$attr$group == 1)
    }
    if (dat$control$type == "SIR") {
      ...
      dat$epi$num <- dat$epi$s.num +
                     ...
                     dat$epi$r.num +
                     dat$epi$f.num
    } else if (dat$control$type == "SEIQHRPAF") {
      dat$epi$num <- dat$epi$s.num +
                     dat$epi$e.num +
                     dat$epi$i.num +
                     dat$epi$q.num +
    ...
    if (dat$control$type %in% c("SEIQHRFPA")) {
      dat$epi$p.num[at] <- sum(dat$attr$active == 1 &
                                 dat$attr$status == "p" &
                                 dat$attr$group == 1)
      dat$epi$a.num[at] <- sum(dat$attr$active == 1 &
                                 dat$attr$status == "a" &
                                 dat$attr$group == 1)
    }
    if (dat$control$type == "SIR") {
        ...
        dat$epi$num[at] <- dat$epi$s.num[at] +
                             ...
                             dat$epi$r.num[at] +
                             dat$epi$f.num[at]
    } else if (dat$control$type == "SEIQHRFPA") {
      dat$epi$num[at] <- dat$epi$s.num[at] +
        dat$epi$e.num[at] +
        dat$epi$i.num[at] +
        dat$epi$q.num[at] +
